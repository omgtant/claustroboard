...
function logTurn(turnNumber: number, player: Player, pos: Pos) {
	const el = logMessage(
		`${turnNumber.toString().padStart(3, ".")}: Player ${
			player.nickname
...
function clearHighlights() {
	if (!board) throw new Error("Board element not found");
	const highlightedTiles = board.querySelectorAll(
		".tile-valid, .tile-selected"
	);
...
function refreshTile(tile: Tile) {
	...
	tileElement.className = `
        tile tile-type-${rnd(1, 2, undefined, tile)} tile-${
...
function getCenterOfTileRatio(pos: Pos): {
	xPercent: number;
	yPercent: number;
} {
	const tileElement = getElementByPos(pos);
	...
	return {
		xPercent:
			(rect.left - boardRect.left + rect.width / 2) / boardRect.width,
		yPercent:
			(rect.top - boardRect.top + rect.height / 2) / boardRect.height,
	};
}
