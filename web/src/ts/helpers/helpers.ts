...
export function getMockInitialState(): InitialState {
	...
	const possiblePositions = board.flatMap((row) =>
		row
			.filter((tile) => tile.canStartOnMe(board))
...
export function readInitialStateIntoGameState(
...
function readIntialBoardIntoGameBoard(tileSetups: TileSetup[][]): Tile[][] {
	return tileSetups.map((row, y) =>
		row.map((tileSetup, x) => {
			...
			if (!tile)
				throw new Error(
					`Failed to create tile at position (${x}, ${y})`
				);
			return tile;
		})
	);
...
export function isTileReasonableToLandOn(
	tile: Tile,
	state: GameState,
	player: Player
) {
...
export function getNeighbors(state: GameState, tile: Tile): Tile[] {
	...
	directions.forEach((dir) => {
		...
		if (
			newX >= 0 &&
			newX < state.board.tiles[0].length &&
			newY >= 0 &&
			newY < state.board.tiles.length
		) {
...
export function shuffle2DArr<T>(arr: T[][]): T[][] {
	...
	for (let i = totalElements - 1; i > 0; i--) {
		...
		// Swap elements
		[copied[iRow][iCol], copied[jRow][jCol]] = [
			copied[jRow][jCol],
			copied[iRow][iCol],
		];
	}
	...
	return moves
		.map((move: Pos | ValidMove) => {
			if (move instanceof Object && "to" in move && "path" in move) {
				...
				return { to: move as Pos, path: [move as Pos] } as ValidMove & {
					path: Pos[];
				};
			}
