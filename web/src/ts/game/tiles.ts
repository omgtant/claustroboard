...
export class LayoutTile extends Tile {
	...
	availableMoves(state: GameState, player: Player): ValidMove[] {
		return Array.from(
			this.dfs(state, player, 0, this.energy, this, new Set(), [])
		);
	}
	...
	dfs(
		...
		if (cur_depth === target_depth)
			return new Set([
				{ to: tile.position, path: [...path, tile.position] },
			]);
		visited.add(tile);
		...
		getNeighbors(state, tile).forEach((neighbor) => {
			if (
				...
				!neighbor.canLandOnMe(state, player) ||
				!neighbor.isOpen ||
				state.players.some((p) => posEq(p.position, neighbor.position))
			)
				return;
			this.dfs(
				state,
...
export class WildcardTile extends Tile {
	...
	dfs(
		...
		visited.delete(tile);
		if (cur_depth !== 0)
			result.add({ to: tile.position, path: path.slice() }); // Include the current tile as a valid move
		path.pop();
		return result;
	}
...
export class TeleportTile extends Tile {
	...
	availableMoves(state: GameState, player: Player): ValidMove[] {
		...
		state.board.tiles.forEach((row) => {
			row.forEach((tile) => {
				...
				if (
					this.color !== TileColor.COLORLESS &&
				...
				result.push({
					to: tile.position,
					path: [this.position, tile.position],
				});
			});
		});
	...
	onPlayerLanding(state: GameState, player: Player): void {
		...
		if (state.playerTurnIndex === -1)
			state.playerTurnIndex = state.players.length - 1;
	}

	canLandOnMe(state: GameState, player: Player): boolean {
		if (
			!state.board.tiles.flat().some((tile) => {
				return (
		...
		return !(
			state.board.tiles
				.flat()
				.find((tile) =>
					posEq(tile.position, player.position)
				) instanceof TeleportTile
		);
	}
...
export class ZeroTile extends Tile {
	...
	onPlayerLanding(state: GameState, player: Player): void {
		...
		const nextPlayerPos =
			activePlayers[
				(activePlayers.findIndex(
					(pl) => pl.nickname === player.nickname
				) +
					1) %
					activePlayers.length
			].position;
		const nextPlayerTile = state.board.tiles
			.flat()
			.find((tile) => posEq(tile.position, nextPlayerPos));
		...
		activePlayers.forEach((pl, index) => {
			...
			renderInterface.movePlayer(-1, pl, {
				to: pl.position,
				path: [oldPos, pl.position],
			});
		});
		activePlayers[activePlayers.length - 1].position = pos;
		renderInterface.movePlayer(
			-1,
			activePlayers[activePlayers.length - 1],
			{ to: activePlayers[activePlayers.length - 1].position }
		);
	}
